// Loads the dev RSA keypair and derives its JWKS representation.
// The exact PUBLIC_KEY PEM string exported here is what the vulnerable middleware
// (mis)uses as an HMAC key, so the RS256->HS256 confusion attack must sign with the
// byte-identical PEM the auditor reconstructs from the JWKS endpoint.
const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

require('./generate'); // ensure keys exist before we read them

const DIR = __dirname;
const PRIVATE_KEY = fs.readFileSync(path.join(DIR, 'private.pem'), 'utf8');
const PUBLIC_KEY = fs.readFileSync(path.join(DIR, 'public.pem'), 'utf8');

const publicKeyObject = crypto.createPublicKey(PUBLIC_KEY);
const jwk = publicKeyObject.export({ format: 'jwk' }); // { kty, n, e }

const JWKS = { ...jwk, kid: 'vuln-key-1', use: 'sig', alg: 'RS256' };

module.exports = { PRIVATE_KEY, PUBLIC_KEY, JWKS };
